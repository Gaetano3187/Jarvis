...
/* --------------------------- Rilevamento colonne "finances" --------------------------- */
let __FIN_COLS = null;

async function tableHasCol(col) {
  const { error } = await supabase.from('finances').select(col).limit(1);
  return !error;
}
async function getFinCols() {
  if (__FIN_COLS) return __FIN_COLS;

  const dateCandidates = ['spent_date','date','spent_on','data','day'];
  const tsCandidates   = ['spent_at','created_at','timestamp','ts','time'];
  const userCandidates = ['user_id','userId','uid'];
  const textCandidates = ['description','desc','title','note','name','prodotto','item'];
  const amountCandidates = ['amount','value','price','cost'];
  const pmCandidates   = ['payment_method','paymentMethod','pay_method','metodo_pagamento'];
  const catIdCandidates = ['category_id','categoryId'];
  const catSlugCandidates = ['category_slug','category','categoria'];

  const pick = async (arr)=>{ for (const c of arr){ if (await tableHasCol(c)) return c; } return null; };

  const dateCol   = await pick(dateCandidates);
  const tsCol     = await pick(tsCandidates);
  const userKey   = await pick(userCandidates);
  const textCol   = await pick(textCandidates);
  const amountCol = await pick(amountCandidates);
  const pmCol     = await pick(pmCandidates);
  const catIdCol  = await pick(catIdCandidates);
  const catSlugCol= await pick(catSlugCandidates);

  __FIN_COLS = { dateCol, tsCol, userKey, textCol, amountCol, pmCol, catIdCol, catSlugCol };
  if (!dateCol && !tsCol) {
    console.warn('[brainHub] Nessuna colonna data/timestamp trovata su "finances". Le query useranno range vuoti.');
  }
  if (!userKey) {
    console.warn('[brainHub] Nessuna colonna utente trovata su "finances" (user_id / userId).');
  }
  if (!textCol) {
    console.warn('[brainHub] Nessuna colonna di descrizione testuale trovata su "finances".');
  }
  if (!amountCol) {
    console.warn('[brainHub] Nessuna colonna importo trovata su "finances".');
  }
  return __FIN_COLS;
}
function getRowDateValue(row, cols) {
  const k = cols.dateCol || cols.tsCol;
  return k ? row[k] : null;
}
function toTsStart(isoDay) { return `${isoDay}T00:00:00Z`; }
function toTsEnd(isoDay)   { return `${isoDay}T23:59:59Z`; }

/* --------------------------- Query/Insert finances (auto-safe) --------------------------- */
async function sumFinancesBetween(userId, startISO, endISO){
  const cols = await getFinCols();
  let q = supabase.from('finances').select(cols.amountCol || '*');
  if (cols.userKey) q = q.eq(cols.userKey, userId);

  if (cols.dateCol)       q = q.gte(cols.dateCol, startISO).lte(cols.dateCol, endISO);
  else if (cols.tsCol)    q = q.gte(cols.tsCol, toTsStart(startISO)).lte(cols.tsCol, toTsEnd(endISO));

  const { data, error } = await q;
  if (error) throw error;
  return (data||[]).reduce((t,r)=> t + Math.abs(Number(cols.amountCol ? r[cols.amountCol] : r.amount || 0)), 0);
}
...
async function sumCategoryBetween(userId, categorySlug, startISO, endISO){
  ...
  if (cols.catIdCol || cols.catSlugCol) {
    const catId = CATEGORY_IDS[categorySlug] || null;
    if (catId && cols.catIdCol) q = q.eq(cols.catIdCol, catId);
    else if (cols.catSlugCol)   q = q.eq(cols.catSlugCol, categorySlug);
  }
...
async function listFinancesBetween(userId, startISO, endISO){
  ...
  if (cols.dateCol) {
    q = q.gte(cols.dateCol, startISO).lte(cols.dateCol, endISO).order(cols.dateCol, { ascending: true, nullsFirst:false });
  } else if (cols.tsCol) {
    q = q.gte(cols.tsCol, toTsStart(startISO)).lte(cols.tsCol, toTsEnd(endISO)).order(cols.tsCol, { ascending: true, nullsFirst:false });
  }

  const { data, error } = await q;
  if (error) throw error;
  return (data||[]).map(r => ({
    description: cols.textCol ? r[cols.textCol] : (r.description || r.desc || r.title || ''),
    amount: Number(cols.amountCol ? r[cols.amountCol] : r.amount || 0),
    _date: getRowDateValue(r, cols),
    __raw: r,
  }));
}

/* --------------------------- Altre tabelle (invariato) --------------------------- */
...
async function computeDisponibileETasca(userId, periodStart, periodEnd){
  ...
  // spese in contanti nel periodo (dinamico su colonne)
  const cols = await getFinCols();
  let fq = supabase.from('finances').select('*').eq(cols.userKey, userId);
  if (cols.dateCol) fq = fq.gte(cols.dateCol, periodStart).lte(cols.dateCol, periodEnd);
  else if (cols.tsCol) fq = fq.gte(cols.tsCol, toTsStart(periodStart)).lte(cols.tsCol, toTsEnd(periodEnd));
  const { data: finAll } = await fq;

  const textCol = cols.textCol;
  const pmCol = cols.pmCol;
  const isElectronicByText = (d)=> ELECTRONIC_TOKENS.some(k=>String(d||'').toLowerCase().includes(k));
  const isCashByFields = (row)=>{
    const pm = pmCol ? String(row[pmCol]||'').toLowerCase() : '';
    if (pm==='cash'||pm==='contanti') return true;
    if (pm && pm!=='cash' && pm!=='contanti') return false;
    return isElectronicByText(textCol ? row[textCol] : (row.description||''));
  };
  const amountKey = cols.amountCol || 'amount';
  const spesePeriodo = (finAll||[]).filter(isCashByFields).reduce((t,r)=>t+Math.abs(Number(r[amountKey]||0)),0);
...
async function searchFinancesByProduct(userId, phrase, { start, end } = {}){
  const cols = await getFinCols();
  if (!cols.textCol) return []; // nessuna colonna testuale → non posso cercare
  let q = supabase.from('finances').select('*');
  if (cols.userKey) q = q.eq(cols.userKey, userId);
  if (start || end) {
    if (cols.dateCol) {
      if (start) q = q.gte(cols.dateCol, start);
      if (end)   q = q.lte(cols.dateCol, end);
    } else if (cols.tsCol) {
      if (start) q = q.gte(cols.tsCol, toTsStart(start));
      if (end)   q = q.lte(cols.tsCol, toTsEnd(end));
    }
  }
  const tokens = tokenizePhrase(phrase);
  if (tokens.length) tokens.forEach(tok => { q = q.ilike(cols.textCol, `%${tok}%`); });
  else if (phrase)   q = q.ilike(cols.textCol, `%${phrase}%`);
  q = q.order(cols.dateCol || cols.tsCol || 'id', { ascending:true });
...
/* ---- Parsing date naturale: oggi/ieri/gg/mm/aaaa/questo mese ---- */
...
async function ingestTextExpense(text){
  ...
  const spent_day = isoLocal(new Date());
  ...
  const cols = await getFinCols();
  const row = {};
  if (cols.userKey) row[cols.userKey] = user.id;
  if (cols.textCol) row[cols.textCol] = description;
  if (cols.amountCol) row[cols.amountCol] = amount;
  if (cols.dateCol)   row[cols.dateCol]   = spent_day;
  if (cols.tsCol)     row[cols.tsCol]     = `${spent_day}T12:00:00Z`;
  if (cols.pmCol)     row[cols.pmCol]     = payment_method;
  if (cols.catIdCol && category_id) row[cols.catIdCol] = category_id;
  else if (cols.catSlugCol)         row[cols.catSlugCol] = category_slug;

  const { error } = await supabase.from('finances').insert(row);
  if (error) { console.error('[finances insert error]', error); throw error; }

  return { ok:true, result:`✅ Spesa registrata${category_slug ? ` in "${category_slug}"` : ''}.` };
}
...
export async function ingestOCRLocal({ file, files, base64 } = {}) {
  let pick = [];
  if (files) { if (Array.isArray(files)) pick = files.filter(Boolean); else if (typeof files.length === 'number') pick = Array.from(files); }
  ...
  if (!pick.length) throw new Error('OCR: nessun file/immagine valido.');

  const fd = new FormData();
  for (const f of pick) { const ok = f && typeof f==='object' && ('size' in f) && ('type' in f); if (ok) fd.append('images', f); }
  ...
  if (amount != null) {
    ...
    const row = {};
    if (cols.userKey)      row[cols.userKey] = user.id;
    if (cols.textCol)      row[cols.textCol] = 'Scontrino supermercato (OCR)';
    if (cols.amountCol)    row[cols.amountCol] = amount;
    if (cols.dateCol)      row[cols.dateCol] = spent_day;
    if (cols.tsCol)        row[cols.tsCol]   = `${spent_day}T12:00:00Z`;
    if (cols.pmCol)        row[cols.pmCol]   = 'cash';
    if (cols.catIdCol)     row[cols.catIdCol] = CATEGORY_IDS['spese-casa'] || null;
    else if (cols.catSlugCol) row[cols.catSlugCol] = 'spese-casa';

    const { error: dbErr } = await supabase.from('finances').insert(row);
    if (dbErr) { console.error('[finances OCR insert error]', dbErr); throw new Error(dbErr.message || 'Insert fallito su finances'); }
  }

  // (Opzionale) riempie lista con prodotti noti
  ...
  return { ok: true, result: '✅ Scontrino riconosciuto e registrato.' };
}
...
function fmtRiga(r) {
  ...
  const dStr = d && !isNaN(d) ? d.toLocaleDateString('it-IT') : '';
  return `• ${dStr} — ${r.description}: € ${Number(r.amount).toFixed(2)}`;
}

/* --------------------------- Router testuale/vocale --------------------------- */
export async function runQueryFromTextLocal(text, { first=false } = {}){
  ...
  if (nlu && nlu.intent){
    try {
      ...
      if (nlu.intent === 'query_product_best_price'){
        ...
        return { ok:true, result:`Prezzo più basso per "${phrase}": € ${Number(best.amount).toFixed(2)} il ${new Date(best._date||best.__raw?.spent_date).toLocaleDateString('it-IT')} — "${best.description}".` };
      }

      // intenti pre-esistenti
      switch(nlu.intent){
        case 'expense_add': {
          ...
          const row = {};
          if (cols.userKey)   row[cols.userKey] = user.id;
          if (cols.textCol)   row[cols.textCol] = desc;
          if (cols.amountCol) row[cols.amountCol] = amount;
          if (cols.dateCol)   row[cols.dateCol] = spent_date;
          if (cols.tsCol)     row[cols.tsCol]   = `${spent_date}T12:00:00`;
          if (cols.pmCol)     row[cols.pmCol]   = pm;
          if (cols.catIdCol && catId) row[cols.catIdCol] = catId;
          else if (cols.catSlugCol)   row[cols.catSlugCol] = catS;
        ...
        case 'query_list_today': {
          ...
          const rows = items.map(i => `• ${i.name}${i.brand?` (${i.brand})`:''} — ${i.qty||i.packs||1} conf. × ${i.unitsPerPack||1} ${i.unitLabel||'unità'}`).join('\n');
  ...
  /* --------- Fallback locali anche per prodotto/date --------- */
