...
function buildNLUPrompt(userText){
  return [
    ...
    '  "date": "YYYY-MM-DD",              // per query_purchases_on_date',
    '  "month": "YYYY-MM",                // per query_purchases_month',
    '  "product": "prosciutto san daniele|parma|...", // per ricerche prodotto',
    ...
    '- “cerca prosciutto san daniele / trova prosciutto parma / elenca prosciutto” → query_product_purchases con product',
    '- “dove l’ho pagato di meno il prosciutto san daniele?” → query_product_best_price con product',
    'ESEMPI:',
    ' "cerca prosciutto san daniele" → {"intent":"query_product_purchases","product":"prosciutto san daniele"}',
    ' "trova parma" → {"intent":"query_product_purchases","product":"parma"}',
...
function parseItalianDateOrKeyword(t){
  ...
  if (/\boggi\b/.test(str))   return { type:'day',  ...dayRange(new Date()) };
  if (/\bieri\b/.test(str))   return { type:'day',  ...dayRange(new Date(Date.now()-24*3600*1000)) };
  if (/l.?altro\s+ieri/.test(str)) return { type:'day', ...dayRange(new Date(Date.now()-2*24*3600*1000)) };
  if (/quest[oa]\s+mese|\bmese\s+corrente\b/.test(str)) return { type:'month', ...monthRange(new Date()) };
  ...
  if (m){
    ...
    return { type:'day', ...dayRange(new Date(d)) };
  }
  return null;
}
