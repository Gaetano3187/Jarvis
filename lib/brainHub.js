...
function monthRange(d=new Date()){
  // da inizio mese FINO A OGGI
  const y=d.getFullYear(), m=d.getMonth();
  return { start: isoLocal(new Date(y,m,1)), end: isoLocal(d) };
}
...
async function sumFinancesBetween(userId, startISO, endISO){
  const dateStartTS = `${startISO}T00:00:00`;
  const dateEndTS   = `${endISO}T23:59:59`;
  const orClause = `and(spent_date.gte.${startISO},spent_date.lte.${endISO}),and(spent_at.gte.${dateStartTS},spent_at.lte.${dateEndTS})`;
...
async function sumCategoryBetween(userId, categorySlug, startISO, endISO){
  ...
  let q = supabase.from('finances').select('amount')
    .eq('user_id', userId)
    .or(orClause);
...
async function computeDisponibileETasca(userId, periodStart, periodEnd){
  ...
  const dateStartTS = `${periodStart}T00:00:00`;
  const dateEndTS   = `${periodEnd}T23:59:59`;
  const orClause = `and(spent_date.gte.${periodStart},spent_date.lte.${periodEnd}),and(spent_at.gte.${dateStartTS},spent_at.lte.${dateEndTS})`;
...
function buildNLUPrompt(userText){
  return [
    ...
    '  "amount": 0, "description": "", "payment_method": "cash|card|", "category_suggestion": "spese-casa|vestiti-ed-altro|cene-aperitivi|varie|",',
    '  "months": 2, "category": "cene-aperitivi|spese-casa|vestiti-ed-altro|varie|",',
    '  "items": [ { "name":"latte", "qty":2, "mode":"packs|units", "unitLabel":"bottiglie|unità|vasetti|...", "brand":"" } ],',
    '  "updates": [ { "name":"latte", "value":3, "mode":"packs|units" } ],',
    '  "expiries": [ { "name":"latte", "expiresAt":"YYYY-MM-DD" } ],',
    '  "product": "latte", "listType": "supermercato|online|"',
    '}',
    'REGOLE:',
    '- Supporta "2 pacchi", "3 bottiglie", "4 unità", numeri in lettere.',
    '- SOLO JSON, nessun testo fuori dal JSON.',
    '- Non inventare importi; se assenti, ometti "amount".',
    '- category_suggestion tra: spese-casa, vestiti-ed-altro, cene-aperitivi, varie.',
    '- listType default: "supermercato".',
    'TESTO UTENTE:',
    userText
  ].join('\n');
}
...
function parseUnitsHint(chunk){
  ...
  if (/unit|pz|pezzi|vasett|uova|barrett|merendine|bustine|monouso/.test(c)) return { unitLabel:'unità', mode:'units' };
...
export async function runQueryFromTextLocal(text, { first=false } = {}){
  ...
  // 3) Fallback locale (regex euristici)
  ...
  if (/quanto.*(spes[oa]|uscit[ae]).*(questo|quest’|sto)\s*mese/.test(t)) {
    const user = await requireUser();
    const total = await sumFinancesBetween(user.id, ms, me);
    return { ok:true, result:`Hai speso € ${total.toFixed(2)} nel mese corrente.` };
  }
  if (/(cene|ristorant|aperitiv|bar).*(ultim[ei]|scorsi).*(mes)/.test(t)) {
  ...
  if (/(saldo|disponibil|in\s+tasca)/.test(t)) {
    const user = await requireUser();
    const s = await computeDisponibileETasca(user.id, ms, me);
    return { ok:true, result: `Saldo disponibile: € ${s.saldoDisponibile.toFixed(2)} • Soldi in tasca: € ${s.pocketBalance.toFixed(2)}.` };
  }
