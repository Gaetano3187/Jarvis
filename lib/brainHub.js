...
return {
  ...
  summary: {
    totale: total,
    "in_scadenza_<=3gg": expSoon,   // << chiave quotata
    stati: { low, med, ok }
  },
  elenco: items,
  note: onlyLow
...
/* ========================== CATEGORIE ========================== */
/**
 * Sinonimi → categorie. Personalizzali con i nomi reali presenti in finance_categories.name
 * Esempi comuni: "Casa", "Cene e aperitivi", "Vestiti/Abbigliamento", "Varie" ecc.
 */
const CAT_ALIASES = {
...
/** Ritorna lista di id categoria coerenti con un alias (“casa”, “cene”, …) */
...
/* ====================== INTENT PARSER ======================= */
/** Riconosce “Casa/Cene/Vestiti/Varie” */
...
/* ======================= PUBLIC API ======================== */
/** Interpreta testo libero (usato in Home) */
export async function runQueryFromTextLocal(text, { first=false } = {}){
  ...
  try {
    switch (intent.type) {
      ...
      case 'WHERE_BUY': {
        const res = await suppliersFor({ userId, range, term: intent.term }); // term=null → tutti
        return { ok:true, result: res };
      }
