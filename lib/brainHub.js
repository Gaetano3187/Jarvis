...
export async function ingestOCRLocal({ file, files, base64 }) {
  // ---- 1) Prepara FormData per /api/ocr (richiede 'images')
  ...
  if (file instanceof File) {
    ...
    // converte base64 -> Blob per compatibilità con /api/ocr
    const blob = b64ToBlob(base64, 'image/jpeg');
    fd.append('images', new File([blob], 'ocr.jpg', { type: 'image/jpeg' }));
  } else {
  ...
  if (!ocrRes.ok || ocrJson?.error) {
    throw new Error(ocrJson?.error || `OCR HTTP ${ocrRes.status}`);
  }
  const ocrText = String(ocrJson?.text || '').trim();
  if (!ocrText) throw new Error('OCR: risposta vuota.');

  // ---- 3) Tenta di trovare un "TOTALE" sullo scontrino
  ...
  // ---- 4) Inserisci su finances (categoria: Spese Casa)
  if (amount != null) {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error('Sessione scaduta');

    const spent_date = isoLocal(new Date());           // YYYY-MM-DD
    const payload = {
      user_id: user.id,
      description: 'Scontrino supermercato (OCR)',
      amount,
      // usa sia la data "pulita" sia un timestamp stabile a mezzogiorno UTC
      spent_date,
  ...
  // ---- 5) (Opzionale) Aggiungi alcuni articoli riconosciuti alla Lista Supermercato
...
// Helper locale (mettilo vicino agli altri helpers in brainHub.js)
