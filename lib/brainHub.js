...
/* ========================== SCORTE ========================== */
/**
 * Ritorna snapshot scorte **con nome reale** del prodotto.
 * - Leggo da `scorte_view` (metrica, fill/status, ecc.)
 * - In parallelo mappo `inventory.id -> products.name` per sostituire eventuali segnaposto
 */
async function scorteOverview({ userId, onlyLow=false, scadenzaGiorni=null }){
  ...
  // inventory.id → products.name (se esiste)
  ...
  const rows = (viewRows||[]).map(r => {
    // se la view ha un nome placeholder tipo "Prodotto abcd…" sostituisco col nome vero
  ...
  return {
    ...
    summary: {
      totale: total,
      "in_scadenza_<=3gg": expSoon,   // chiave quotata per TS
      stati: { low, med, ok }
    },
    elenco: items,
    note: onlyLow
