...
/* === Parser unità con sinonimi (bottiglie/rotoli/scatole/vasetti/…) === */
function parseUnitsHint(chunk){
  const c = String(chunk || '').toLowerCase();

  // Confezioni/pacchi/rotoli/scatole → packs = qty, unitsPerPack = 1
  ...
  // Unitari (vasetti/uova/pezzi/…) → packs = 1, unitsPerPack = qty
...
/* =================== STAGING LISTA locale (quando il brain non c’è) =================== */
...
function listProxyAdd(items, listType='supermercato') {
  const prev = readLocalList();
  const next = [...prev];
  for (const raw of (items||[])) {
    const it = normalizeItemForList(raw, listType);
    const hitIdx = next.findIndex(x =>
      x.name.toLowerCase() === it.name.toLowerCase()
      && (x.brand||'') === (it.brand||'')
      && (x.unitLabel||'unità') === (it.unitLabel||'unità')
    );
    if (hitIdx >= 0) {
      next[hitIdx].packs = Math.max(1, Number(next[hitIdx].packs||1)) + Math.max(1, Number(it.packs||1));
    } else {
      next.push(it);
    }
  }
...
async function addItemsToList(items, listType='supermercato'){
  const hb = getBrain();
  const normalized = (items || []).map(it => normalizeItemForList(it, listType));

  if (!hb?.run) {
    const added = listProxyAdd(normalized, listType);
    return `Aggiunti ${added} elemento/i alla lista ${listType} (in attesa di sincronizzazione).`;
  }

  for (const it of normalized) {
    await hb.run('aggiungi-alla-lista', it);
  }
  return `Aggiunti ${normalized.length} elemento/i alla lista ${listType}.`;
}
