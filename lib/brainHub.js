...
/* ========================== SCORTE ========================== */
/**
 * Snapshot scorte con nome reale del prodotto (se disponibile).
 * - scorte_view: id (inventory), name (placeholder o reale), qty, unit, fill_pct, consumed_pct_calc, status, days_to_expiry
...
async function scorteOverview({ userId, onlyLow=false, scadenzaGiorni=null }){
  ...
  const [{ data: viewRows, error: e1 }, { data: invRows, error: e2 }] = await Promise.all([
    q.order('fill_pct', { ascending:true, nullsLast:true }),
    supabase.from('inventory').select('id,product_id').eq('user_id', userId)
  ]);
  if (e1) throw e1;
  if (e2) throw e2;

  // product_id → name
  const prodIds = Array.from(new Set((invRows||[]).map(r=>r.product_id).filter(Boolean)));
  let prodNameById = {};
  if (prodIds.length) {
    const { data: prods, error: e3 } = await supabase
      ...
      .in('id', prodIds);
    if (e3) throw e3;
    prodNameById = Object.fromEntries((prods||[]).map(p => [p.id, p.name]));
  }

  // inventory.id → products.name
  const invIdToProdName = {};
  for (const inv of (invRows||[])) {
    if (inv.product_id && prodNameById[inv.product_id]) {
      invIdToProdName[inv.id] = prodNameById[inv.product_id];
    }
  }

  const rows = (viewRows||[]).map(r => {
    // Se name è un placeholder tipo "Prodotto 46f78c…" prova a sostituire
    const maybePlaceholder = /^prodotto\s+[0-9a-f]{4,}$/i.test(String(r.name || ''));
    const resolvedName = invIdToProdName[r.id] || (!maybePlaceholder ? r.name : null);
    return { ...r, name: resolvedName || r.name || '—' };
  });
...
// Fallback noto dalla tua pagina spese-casa
...
export async function runQueryFromTextLocal(text, { first=false } = {}){
  ...
  try {
    switch (intent.type) {
      ...
      /* --- fallback help: **stringa**, non oggetto --- */
      default:
        return {
          ok:true,
          result:
`💡 Cose che puoi chiedermi:
• Quanto ho speso **questo mese**?
• Quanto ho speso **per casa** / **cene** / **vestiti**?
• In quali prodotti spendo di più (**per casa**, ecc.)?
• Dove compro **il latte** (anche per categoria: **per cene**)?
• Cosa ho a casa / scorte che stanno finendo / in scadenza?`
        };
    }
  } catch (e) {
