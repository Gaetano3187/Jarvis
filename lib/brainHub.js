...
/* --------------------------- OCR ingest --------------------------- */
export async function ingestOCRLocal({ base64 }){
  if (!base64) throw new Error('OCR: immagine non valida');
  ...
  // Registra spesa totale come Spese Casa (se trovo "TOTALE")
  const lines = String(text||'').split(/\r?\n/).map(s=>s.trim()).filter(Boolean);
  const totalGuess = lines.find(l=>/totale/i.test(l));
  const mm = totalGuess?.match(/(\d{1,3}(?:[.,]\d{3})*(?:[.,]\d{1,2})?)/);
  ...
  if (amount != null) {
    ...
    const payload = {
      ...
      spent_at: `${spent_date}T12:00:00`,
      payment_method: 'cash',
      category_slug: CATEGORY_SLUGS.CASA,
      category_id: CATEGORY_IDS[CATEGORY_SLUGS.CASA] || null
    };
    const { error } = await supabase.from('finances').insert(payload);
    if (error) throw error;
  }

  // Estrai voci e caricale in lista (base euristica + nomi)
  const picks = lines
  ...
  return { ok:true, result:'✅ Scontrino registrato su Spese Casa.' };
}
