...
export function answerFromHub(prompt, hub) {
  const text = String(prompt || '').toLowerCase();
  const fmt = (n) => `€ ${Number(n || 0).toFixed(2)}`;

  // ===== ENTRATE & SALDI =====
  if (/(saldo|disponibilit[aà]|quanto ho (disponibil|in tasca|libero)|soldi (disponibili|rimasti|che ho))/.test(text)) {
    const val = hub?.entrate_saldoDisponibile ?? hub?.saldoDisponibile ?? 0;
    return `Ciao Gaetano, il saldo disponibile è ${fmt(val)}.`;
  }

  if (/(quanto ho speso|spese.*(mese|periodo|corrente)|speso finora|speso questo mese)/.test(text)) {
    const val = hub?.entrate_speseMese ?? hub?.speseMese ?? 0;
    return `Ciao Gaetano, nel periodo corrente hai speso ${fmt(val)}.`;
  }

  if (/(soldi in tasca|contanti rimasti|quanto ho in tasca)/.test(text)) {
    const val = hub?.entrate_pocketBalance ?? hub?.pocketBalance ?? 0;
    return `Hai in tasca ${fmt(val)}.`;
  }

  if (/(quali entrate|lista entrate|che entrate ho|mi elenchi le entrate)/.test(text)) {
    const rows = hub?.entrate_list || [];
    if (!rows.length) return 'Non ho trovato entrate nel periodo.';
    const lines = rows.slice(0, 10).map(r => {
      const d = String(r.date || '').slice(0,10).split('-').reverse().join('/');
      return `• ${d} — ${r.source}: ${fmt(r.amount)} (${r.description || '—'})`;
    });
    return `Entrate recenti:\n${lines.join('\n')}`;
  }

  return null; // fallback al resto del cervello
}
