...
// --- SOSTITUISCI toolStockSnapshot ESISTENTE ---
async function toolStockSnapshot({ userId }) {
  const uid = await getUserIdOrThrow(userId);

  // Prova in ordine: stock "puro", poi la lista prodotti della tua UI
  const candidates = [
    // stock classici
    { table: 'jarvis_stock', cols: {} },
    { table: 'stock',        cols: {} },
    { table: 'scorte',       cols: {} },
    // lista prodotti usata in UI (dove vedi le barre verdi)
    // spesso i campi sono: name/prodotto, residuo, qty/quantity/qta, fill_pct/consumo_pct/remaining_pct
    { table: 'jarvis_liste_prodotti', cols: {} },
    { table: 'liste_prodotti',        cols: {} },
  ];
  ...
  for (const c of candidates) {
    ...
    const mapped = rows.map(r => {
      ...
      // percentuale riempimento/consumo
      let fill =
        r.fill_pct ?? r.remaining_pct ?? r.consumo_pct ?? r.level_pct ?? null;

      // Se non cՏ fill_pct, prova a calcolarla se abbiamo totali + residuo
      // esempi di possibili colonne: packs, unitsPerPack, total_units, initial_units, consumed_units
    ...
    // se abbiamo almeno un elemento sensato, ritorna
    const nonEmpty = mapped.filter(x => x.name && (x.qty != null || x.fill_pct != null || x.expires_at));
    if (nonEmpty.length) {
      // ordina per % riempimento crescente se disponibile, altrimenti per nome
      nonEmpty.sort((a, b) => {
      ...
      return { kind: 'inventory.snapshot', elenco: nonEmpty };
    }
  }

  // Nessuna tabella con dati utili
