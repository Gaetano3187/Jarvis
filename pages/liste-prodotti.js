...
useEffect(() => {
  ...
  try {
    ...
    if (!expPairs.length && !updates.length) {
      ...
      // Riepilogo compatto nel toast
      const MAX = 3;
      const head = msgParts.slice(0, MAX).join(' · ');
      const extra = msgParts.length > MAX ? ` (+${msgParts.length - MAX})` : '';
      showToast(`Inventario aggiornato ✓  ${head}${extra}`, 'ok');
    }
  } catch (e) {
