...
export default function ListeProdotti() {
  ...
  return (
    ...
    {/* Scorte complete — VISUALIZZAZIONE A RIGHE */}
  ...
  {stock.length === 0 ? (
    ...
    <div style={styles.stockTable}>
      ...
      <div style={styles.stockHeader}>
        <div style={{ gridColumn: '1 / span 2' }}>Prodotto</div>
        <div style={{ textAlign:'right' }}>Confezioni</div>
        <div style={{ textAlign:'right' }}>Unità/conf.</div>
      </div>
      ...
      {stock.map((s, idx) => {
        ...
        if (editingRow === idx) {
          return (
            <div key={idx} style={{ ...styles.stockRowLine, ...(idx % 2 ? styles.zebra2 : styles.zebra1) }}>
              <div style={styles.imageBox} />
              <div style={{ flex:1, minWidth:0 }}>
                <div style={styles.formRowWrap}>
                  <input style={styles.input} value={editDraft.name}
                         onChange={e=>handleEditDraftChange('name', e.target.value)} placeholder="Prodotto" />
              ...
              <div style={styles.kvCol}><div style={styles.kvValue}>{Number(editDraft.packs||0)}</div></div>
              <div style={styles.kvCol}><div style={styles.kvValue}>{Number(editDraft.unitsPerPack||1)}</div></div>
            </div>
          );
        }
        ...
        return (
          <div key={idx} style={{ ...styles.stockRowLine, ...(idx % 2 ? styles.zebra2 : styles.zebra1) }}>
            {/* Colonna immagine (clic per caricare) */}
            <div
            ...
            {/* Nome + barra consumo + info scadenza */}
            <div style={{ flex:1, minWidth:0 }}>
              <div style={styles.stockTitle}>
                {s.name}{s.brand ? <span style={styles.rowBrand}> · {s.brand}</span> : null}
              </div>
