...
/** Estrae coppie {name, qty} da frasi tipo:
 *  "latte sono 3 bottiglie, pasta 4 pacchi, ferrero fiesta 3 unità"
 *  "latte = 3", "latte 3", "ho latte 3", ecc.
 */
function parseStockUpdateText(text) {
  const t = normKey(text);
  // separa per virgole o connettivi
  const parts = t.split(/[,;]+|\be\b|\band\b/).map(s=>s.trim()).filter(Boolean);
  const results = [];
  const UNIT = '(?:bottiglie?|bott|btl|pz|pezzi|unita|unità|conf(?:e(?:zioni)?)?|pacchi?|scato(?:le)?|barrette|vasetti?)?';

  for (let chunk of parts) {
    // prova pattern esplicito "XXX sono/è = NUM unità"
    let m = chunk.match(new RegExp(`^(.*?)\\s*(?:sono|e'|è|=)?\\s*(\\d+(?:[.,]\\d+)?)\\s*${UNIT}$`, 'i'));
    if (!m) {
      // prova "XXX NUM [unità]"
      m = chunk.match(new RegExp(`^(.*?)\\s*(\\d+(?:[.,]\\d+)?)\\s*${UNIT}$`, 'i'));
    }
    if (m) {
      let name = (m[1] || '').trim();
      // ripulisci nome da parole di riempimento
      name = name.replace(/\b(ho|di|della|del|dei|le|la|i|il|uno|una|un)\b/g,' ').replace(/\s{2,}/g,' ').trim();
      const qty = Number(String(m[2]).replace(',','.')) || 0;
      if (name && qty >= 0) results.push({ name, qty });
      continue;
    }

    // fallback: solo nome → ignora
  }

  // tenta normalizzazione sul lessico
  const normalized = results.map(r => {
    let best = r.name;
    let bestScore = 0;
    for (const lex of GROCERY_LEXICON) {
      if (isSimilar(r.name, lex)) {
        const score = lex.length;
        if (score > bestScore) { best = lex; bestScore = score; }
      }
    }
    return { name: best, qty: r.qty };
  });

  if (DEBUG) console.log('[parseStockUpdateText]', normalized);
  return normalized;
}
...
export default function ListeProdotti() {
  ...
  // Scorte & critici
  // Estendo il record scorta: {name, brand, qty, expiresAt?, baselineQty?, lastRestockAt?, avgDailyUse?}
  ...
  // Vocale: AGGIORNA SCORTE (session dedicata)
  ...
  useEffect(() => {
    ...
    const crit = stock.filter((p) => {
      ...
      // Consumo >80% → rimasto ≤20% della baseline
      const lowPercent = baseline > 0 ? (qty <= baseline * 0.2) : false;
  ...
  // ✅ Segna acquistato (1 o tutta la quantità) + aggiorna scorte/baseline/lastRestock
  function markBought(id, amount = 1) {
    const item = (lists[currentList] || []).find(i => i.id === id);
    if (!item) return;

    const moveQty = Math.max(1, Math.min(Number(item.qty || 0), Number(amount || 1)));
    ...
    setLists(prev => {
      ...
      next[currentList] = (prev[currentList] || [])
        .map(i => {
          if (i.id !== id) return i;
          const newQty = Math.max(0, Number(i.qty || 0) - moveQty);
    ...
    setStock(prev => {
      ...
      if (idx >= 0) {
        const newQty = Number(arr[idx].qty || 0) + moveQty;
        arr[idx] = {
          ...arr[idx],
          qty: newQty,
          baselineQty: newQty,         // baseline si riallinea al nuovo carico
  ...
  function editStockRow(i) {
    ...
    setStock(prev => {
      const arr = [...prev];
      // aggiorna consumo medio (se ha senso)
      const old = arr[i];
      let avgDailyUse = old?.avgDailyUse || 0;
      if (old?.lastRestockAt && old?.baselineQty > qty) {
        const days = Math.max(1, (Date.now() - new Date(old.lastRestockAt).getTime())/86400000);
        const used = Math.max(0, Number(old.baselineQty) - qty);
        const day = used / days;
        avgDailyUse = avgDailyUse ? (0.6*avgDailyUse + 0.4*day) : day;
      }
      arr[i] = {
        ...old,
        name: name.trim(),
  ...
  async function startVoiceStockUpdate() {
    ...
    try {
      ...
      setUpdRecBusy(true);
      // auto-stop dopo 12s per sicurezza
    ...
    try {
      ...
      setStock(prev => {
        ...
        for (const u of updates) {
          ...
          if (idx < 0) {
            // se non esiste lo creo (aggiornamento “secco” senza considerarlo un carico)
            arr.unshift({
              name: u.name,
              brand: '',
              qty: Math.max(0, Number(u.qty||0)),
              expiresAt: '',
              baselineQty: Math.max(0, Number(u.qty||0)), // baseline iniziale
              lastRestockAt: todayISO,
              avgDailyUse: 0
            });
            applied++;
          ...
          const old = arr[idx];
          const newQty = Math.max(0, Number(u.qty||0));

          // stima consumo giornaliero dalle info disponibili
          let avgDailyUse = old?.avgDailyUse || 0;
          if (old?.lastRestockAt && old?.baselineQty > newQty) {
            const days = Math.max(1, (Date.now() - new Date(old.lastRestockAt).getTime())/86400000);
            const used = Math.max(0, Number(old.baselineQty) - newQty);
            const day = used / days;
            avgDailyUse = avgDailyUse ? (0.6*avgDailyUse + 0.4*day) : day;
          }

          // adatta baseline: non la abbassiamo bruscamente per non falsare i futuri,
          // ma se l'utente dichiara una quantità maggiore della baseline, riallineiamo.
          const baselineQty = Math.max(Number(old.baselineQty||0), newQty);
          ...
          arr[idx] = {
            ...
            baselineQty,
            avgDailyUse
            // NON tocchiamo lastRestockAt: non è un "carico", è un ricalcolo
          };
          applied++;
        }
        return arr;
    ...
    <>
      ...
      <div style={styles.page}>
        <div style={styles.card}>
          ...
          {/* Stato scorte + (tasti OCR/Vocale scadenze/aggiorna scorte) */}
          <div style={styles.sectionXL}>
