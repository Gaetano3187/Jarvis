...
function saveEditRow(rowKey) {
  ...
  <tbody>
    {stock.map(function (s, i) {
      return (
        <tr key={(s && s._key) ? s._key : i}>
          <td style={styles.td}>
            {editingRowKey === i ? (
              <input
                value={(draftRow && draftRow.name) ? draftRow.name : ''}
          ...
          <td style={styles.td}>
            {editingRowKey === i ? (
              <input
                value={(draftRow && draftRow.brand) ? draftRow.brand : ''}
                onChange={(e) => handleDraftChange('brand', e.target.value)}
              />
            ) : (
              (s.brand && s.brand.length ? s.brand : '-')
            )}
          </td>
          ...
          <td style={styles.td}>
            {editingRowKey === i ? (
              <input
                type="number"
                min="0"
                value={(draftRow && typeof draftRow.packs !== 'undefined') ? draftRow.packs : 0}
                onChange={(e) => handleDraftChange('packs', e.target.value)}
              />
            ) : (
              (typeof s.packs === 'number'
                ? s.packs.toFixed(2)
                : (s.packs ? String(s.packs) : '0'))
            )}
          </td>
            ...
            {editingRowKey === i ? (
              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                <input
                  type="number"
                  min="1"
                  value={(draftRow && typeof draftRow.unitsPerPack !== 'undefined') ? draftRow.unitsPerPack : 1}
                  onChange={(e) => handleDraftChange('unitsPerPack', e.target.value)}
                  style={{ width: 60 }}
                />
                <input
                  value={(draftRow && draftRow.unitLabel) ? draftRow.unitLabel : ''}
                  onChange={(e) => handleDraftChange('unitLabel', e.target.value)}
                  style={{ width: 80 }}
                />
              </div>
            ) : (
              ((s && typeof s.unitsPerPack !== 'undefined' ? s.unitsPerPack : 1) +
               ' ' +
               (s && s.unitLabel ? s.unitLabel : 'unità'))
            )}
          </td>
          ...
          <td style={styles.td}>
            {editingRowKey === i ? (
              <input
                type="number"
                min="0"
                value={(draftRow && typeof draftRow.residueUnits !== 'undefined') ? draftRow.residueUnits : 0}
          ...
          <td style={styles.td}>
            {editingRowKey === i ? (
              <input
                type="date"
                value={(draftRow && draftRow.expiresAt) ? draftRow.expiresAt : ''}
