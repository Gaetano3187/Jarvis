...
function SpeseCasa() {
  ...
  const processVoice = async () => {
    ...
    console.log('[stt] Sending audio blob to /api/stt, bytes:', blob.size)
    try {
      const res = await fetch('/api/stt', { method: 'POST', body: fd })
      const { text, error: sttErr } = await res.json()
      if (sttErr) {
        console.error('[stt] Error response:', sttErr)
        setError(`STT ${sttErr}`)
      } else {
        console.log('[stt] Raw transcript:', text)
...
`
  ...
  async function parseAssistantPrompt(prompt) {
    console.log('[assistant] Sending prompt:', prompt)
    try {
      ...
      if (apiErr) {
        console.error('assistant apiErr', apiErr)
