...
function monthBounds(mk){if(mk==='all')return{s:'2000-01-01',e:'2099-12-31'};const[y,m]=mk.split('-').map(Number);return{s:isoLocal(new Date(y,m-1,1)),e:isoLocal(new Date(y,m,0))}}
...
function CeneAperitivi(){
  ...
  const [mk,setMk]=useState(()=>clampMK(typeof window!=='undefined'?localStorage.getItem('__cene_month')||toMK():toMK()))
  ...
  useEffect(()=>{try{localStorage.setItem('__cene_month',mk)}catch{}}, [mk])
  ...
  async function load(uid){
    ...
    try{
      const[ex,pur]=await Promise.all([
        supabase.from('expenses').select('id,store,store_address,amount,purchase_date,description').eq('user_id',uid).eq('category','cene').gte('purchase_date',si).lte('purchase_date',ei).order('purchase_date',{ascending:false}),
        supabase.from('purchase_items').select('id,name,brand,description,price,store,purchase_date').eq('user_id',uid).eq('category','cene').order('purchase_date',{ascending:false})
      ])
  ...
  async function onSubmit(e){
    ...
    try{
      ...
      await supabase.from('expenses').insert({user_id:user.id,category:'cene',store:form.store,description:form.description,amount:parseFloat(form.amount)||0,purchase_date:form.date||isoLocal(),source:'manual'})
  ...
  async function onAddItem(e){
    ...
    try{
      const{data:{user}}=await supabase.auth.getUser();if(!user)throw new Error()
      await supabase.from('purchase_items').insert({user_id:user.id,category:'cene',name:itemForm.name,brand:itemForm.brand||null,description:itemForm.description||null,price:parseFloat(itemForm.price)||0,store:itemForm.store||null,purchase_date:itemForm.date||isoLocal()})
  ...
  async function handleOCR(file){
    ...
    try{
      ...
      if(data.categoria!=='cene')throw new Error(`Categoria "${data.categoria}" — usa la pagina corretta`)
      const{data:{user}}=await supabase.auth.getUser()
      const{data:exp}=await supabase.from('expenses').insert({user_id:user.id,category:'cene',store:data.store||'Cena',store_address:data.store_address||null,amount:parseFloat(data.price_total||0),purchase_date:data.purchase_date||isoLocal(),payment_method:data.payment_method||'unknown',source:'ocr'}).select('id').single()
      if(exp)await supabase.from('receipts').insert({user_id:user.id,expense_id:exp.id,store:data.store||'',purchase_date:data.purchase_date||isoLocal(),price_total:parseFloat(data.price_total||0),payment_method:data.payment_method||'unknown',confidence:data.confidence||'medium'})
      await load(user.id)
      ...
      mr.current.onstop=async()=>{
        ...
        try{
          ...
          if(d.action?.type==='add_expense'){
            const{data:{user}}=await supabase.auth.getUser()
            await supabase.from('expenses').insert({user_id:user.id,category:'cene',store:d.action.store||'Cena/Aperitivo',amount:Number(d.action.amount||0),purchase_date:d.action.date||isoLocal(),source:'voice'})
            await load(user.id)
    ...
    <PageShell title="Cene">
      ...
      <div className="kpi-strip">
        <div className="kpi-card">
          <div className="kpi-label">Spese cene & aperitivi</div>
          <div className="kpi-value" style={{color:'#fbbf24'}}>{eur(totale)}</div>
        </div>
        <div className="kpi-card">
          <div className="kpi-label">N° uscite</div>
          <div className="kpi-value" style={{color:'#22d3ee'}}>{expenses.length}</div>
        </div>
        <div className="kpi-card">
          <div className="kpi-label">Piatti registrati</div>
          <div className="kpi-value" style={{color:'#fb923c'}}>{purchases.length}</div>
        </div>
      </div>

      <div className="main-card">
        <div className="section-hd">
          <span className="section-title">🍽️ Cene & Aperitivi</span>
        </div>

        <div className="tab-row">
          <button className="tab-btn" style={tab==='spese'?{color:'#fbbf24',borderBottom:'2px solid #fbbf24'}:{}} onClick={()=>setTab('spese')}>📋 Spese ({expenses.length})</button>
          <button className="tab-btn" style={tab==='piatti'?{color:'#fbbf24',borderBottom:'2px solid #fbbf24'}:{}} onClick={()=>setTab('piatti')}>🥘 Piatti & Ordini ({purchases.length})</button>
        </div>

        {tab==='spese'&&<div className="month-nav">
          <button className="mn-btn" onClick={()=>{if(mk==='all'){setMk(toMK());return}const[y,m]=mk.split('-').map(Number);setMk(toMK(new Date(y,m-2,1)))}}>‹</button>
          <input type="month" value={mk==='all'?toMK():mk} onChange={e=>setMk(clampMK(e.target.value))} className="mn-input"/>
          <button className="mn-btn" onClick={()=>{if(mk==='all'){setMk(toMK());return}const[y,m]=mk.split('-').map(Number);setMk(toMK(new Date(y,m,1)))}}>›</button>
          <button className="mn-btn-all" onClick={()=>setMk('all')}>Tutti</button>
        </div>}
        ...
        <div className="toolbar">
          ...
          <button className="btn btn-item" onClick={()=>{setShowItemForm(v=>!v);setShowForm(false)}}>{showItemForm?'— Chiudi':'🍕 Aggiungi piatto'}</button>
        </div>

        {showForm&&<form className="inline-form" onSubmit={onSubmit}>
          <div className="form-row">
            <div className="form-field"><label className="form-label">Locale</label><input className="form-input" value={form.store} onChange={e=>setForm(f=>({...f,store:e.target.value}))} placeholder="Ristorante Il Cortile…" required/></div>
          ...
          <div className="form-row">
            <div className="form-field"><label className="form-label">Dettaglio</label><input className="form-input" value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))} placeholder="Pizza + birra…"/></div>
        ...
        {showItemForm&&<form className="inline-form" onSubmit={onAddItem}>
          <div className="form-row">
            <div className="form-field"><label className="form-label">Piatto / Bevanda</label><input className="form-input" value={itemForm.name} onChange={e=>setItemForm(f=>({...f,name:e.target.value}))} placeholder="Pizza margherita, Spritz…" required/></div>
          ...
          <div className="form-row">
            <div className="form-field"><label className="form-label">Locale</label><input className="form-input" value={itemForm.store} onChange={e=>setItemForm(f=>({...f,store:e.target.value}))} placeholder="Ristorante…"/></div>
          ...
          <button type="submit" className="btn btn-save" style={{alignSelf:'flex-start',borderColor:'rgba(251,191,36,.4)',color:'#fbbf24'}}>✓ Aggiungi</button>
        </form>}
           ...
           expenses.map(exp=><div key={exp.id} className="exp-block">
             <div className="exp-row" onClick={()=>loadDetail(exp.id)}>
               <div className="exp-left">
                 <span className="exp-store" style={{color:'#fbbf24'}}>{exp.store||'—'}</span>
                 {exp.description&&<span className="exp-addr">{exp.description}</span>}
                 <span className="exp-date">{exp.purchase_date}</span>
               </div>
               <div className="exp-right">
                 <span className="exp-amt" style={{color:'#fbbf24'}}>{eur(exp.amount)}</span>
             ...
             {expanded===exp.id&&<div className="exp-detail">
               {receiptsMap[exp.id]?.items?.length>0?(<>
                 <div className="detail-label">🍽️ Voci scontrino</div>
                 <div className="items-list">
                   {receiptsMap[exp.id].items.map(it=><div key={it.id} className="it-row">
                     ...
                     <span className="it-price" style={{color:'#fbbf24'}}>{eur(it.price)}</span>
                   </div>)}
                 </div>
        ...
        {tab==='piatti'&&<div className="list-body">
          {purchases.length===0?<div className="list-empty">Nessun piatto registrato — aggiungi con "🍕 Aggiungi piatto"</div>:
           purchases.map(p=><div key={p.id} className="purchase-row">
             <div className="pur-icon">🍽️</div>
             ...
             <div className="pur-meta">
               <span className="pur-price" style={{color:'#fbbf24'}}>{eur(p.price)}</span>
