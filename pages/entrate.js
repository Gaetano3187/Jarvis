...
function formatIT(iso) {
  // iso può essere "YYYY-MM-DD" oppure vuoto
  if (!iso) return '';
...
function Entrate() {
  ...
  const [newIncome, setNewIncome] = useState({
    ...
    description: '',
    amount: '',
    receivedAt: todayLocal, // default alla data odierna locale
  });
  ...
  }, [saldoDisponibile, pocketBalance, entratePeriodo, carryAmount, monthExpenses, startDate, endDate]);
        ...
        <div className="spese-casa-container2">
          ...
          {loading ? <p>Caricamento…</p> : (
            <table className="custom-table">
              ...
              <tbody>
                {incomes.map((i) => {
                  // priorità a received_date (locale), fallback su received_at
