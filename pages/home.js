  ...
  return (
    <>
      ...
      <style jsx global>{`
        ...
        @keyframes cardGlow { 0% { box-shadow: 0 0 15px rgba(var(--glowA), 0.4); } 50% { box-shadow: 0 0 35px rgba(var(--glowB), 0.85); } 100% { box-shadow: 0 0 15px rgba(var(--glowA), 0.4); } }

        /* —— Sheen —— */
        .sheen::before {
          content: ""; position: absolute; inset: -22%; border-radius: inherit;
          background: linear-gradient(75deg, rgba(var(--tint), 0.00) 0%, rgba(var(--tint), 0.10) 28%, rgba(255,255,255, 0.45) 50%, rgba(var(--tint), 0.16) 72%, rgba(var(--tint), 0.00) 100%);
          transform: translateX(-130%) skewX(-12deg); filter: blur(0.6px); mix-blend-mode: screen; pointer-events: none; animation: sweepShine 2.8s ease-in-out infinite;
        }
        .card-finanze.sheen::before { animation-delay: .6s; }
        @keyframes sweepShine { 0% { transform: translateX(-130%) skewX(-12deg); opacity: .65; } 60% { transform: translateX(0%) skewX(-12deg); opacity: 1; } 100% { transform: translateX(130%) skewX(-12deg); opacity: 0; } }
        ...
        /* —— Chat styles —— */
        .chat-overlay { position: fixed; inset: 0; background: rgba(0,0,0,.55); display: grid; place-items: center; z-index: 9999; backdrop-filter: blur(2px); }
        .chat-modal { width: min(920px, 92vw); max-height: 82vh; background: rgba(0,0,0,.78); border: 1px solid rgba(255,255,255,.18); border-radius: 16px; display: grid; grid-template-rows: auto 1fr auto; overflow: hidden; box-shadow: 0 12px 30px rgba(0,0,0,.45); }
        .chat-header { display: flex; align-items: center; justify-content: space-between; padding: .7rem .9rem; background: linear-gradient(145deg, rgba(99,102,241,.28), rgba(6,182,212,.22)); border-bottom: 1px solid rgba(255,255,255,.16); }
        .chat-title { font-weight: 800; color: #fff; }
        .chat-close { background: transparent; color: #fff; border: 1px solid rgba(255,255,255,.25); border-radius: 10px; padding: .2rem .5rem; cursor: pointer; }
        .chat-body { padding: .8rem .9rem; overflow: auto; display: grid; gap: .6rem; background: radial-gradient(1200px 500px at 10% 0%, rgba(236,72,153,.05), transparent 60%), radial-gradient(800px 400px at 100% 100%, rgba(59,130,246,.06), transparent 60%), rgba(0,0,0,.15); }
        .chat-hint { opacity: .85; font-size: .95rem; color: #e5e7eb; }
        .chat-msg { display: grid; }
        .chat-msg.me { justify-content: end; }
        .chat-msg.bot { justify-content: start; }
        .bubble { max-width: 78ch; white-space: pre-wrap; word-break: break-word; background: rgba(255,255,255,.08); border: 1px solid rgba(255,255,255,.18); padding: .55rem .7rem; border-radius: 12px; color: #fff; }
        .chat-msg.me .bubble { background: linear-gradient(145deg, rgba(99,102,241,.45), rgba(6,182,212,.38)); border-color: rgba(255,255,255,.22); }

        .chat-inputrow { display: grid; grid-template-columns: 1fr auto; gap: .6rem; padding: .7rem .9rem; border-top: 1px solid rgba(255,255,255,.16); background: rgba(0,0,0,.35); }
        .chat-input { width: 100%; background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.2); border-radius: .55rem; padding: .55rem .7rem; color: #fff; outline: none; }
        .chat-input::placeholder { color: rgba(255,255,255,.65); }
        .chat-send { background: linear-gradient(135deg, #6366f1, #06b6d4); border: 1px solid rgba(255,255,255,0.2); border-radius: .55rem; padding: .45rem .9rem; color: #fff; cursor: pointer; }
