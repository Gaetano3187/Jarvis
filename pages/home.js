...
function formatResult(res) {
  ...
  if (['string','number','boolean'].includes(typeof res)) return String(res);
...
function svgBars(items,{max=100,unit='%',bg='#0b0f14'}={}) {
  ...
  return `<svg viewBox="0 0 ${W} ${H}" width="100%" height="auto" style="background:${bg};border:1px solid #1f2a38;border-radius:12px">${svgRows}</svg>`;
}

/* ===================================================================
   Date & periodo (per riepiloghi brain)
=================================================================== */
function boundsFromRef(ref='month'){
  ...
  if(ref==='year'){ const s=new Date(now.getFullYear(),0,1); const e=new Date(now.getFullYear(),11,31); return {start:iso(s),end:iso(e)}; }
  const s=new Date(now.getFullYear(),now.getMonth(),1); const e=new Date(now.getFullYear(),now.getMonth()+1,0); return {start:iso(s),end:iso(e)};
}
const refLabel = (ref)=> ref==='today'?'oggi':ref==='week'?'questa settimana':ref==='year'?"quest'anno":'questo mese';

/* ===================================================================
   OCR & normalizzazione (dedupe, unità, ecc.)
=================================================================== */
...
function toISODate(any){
  ...
  if(m1){ const d=String(m1[1]).padStart(2,'0'); const M=String(m1[2]).padStart(2,'0'); let y=String(m1[3]); if(y.length===2) y=(Number(y)>=70?'19':'20')+y; return `${y}-${M}-${d}`; }
  const mesi=['gen','feb','mar','apr','mag','giu','lug','ago','set','ott','nov','dic'];
  const m2=s.toLowerCase().match(/(\d{1,2})\s+([a-zà-ú]+)\s+(\d{2,4})/i);
  if(m2){ const d=String(m2[1]).padStart(2,'0'); const mon=m2[2].slice(0,3); const idx=mesi.indexOf(mon); if(idx>=0){ let y=String(m2[3]); if(y.length===2) y=(Number(y)>=70?'19':'20')+y; const M=String(idx+1).padStart(2,'0'); return `${y}-${M}-${d}`; } }
  return '';
}
function pickDateFromTexts(texts=[]){
  const joined=String((texts||[]).join('\n')||'');
  const m1=joined.match(/(\d{1,2}[\/.-]\d{1,2}[\/.-]\d{2,4})/); if(m1){ const iso=toISODate(m1[1]); if(iso) return iso; }
  const m2=joined.match(/(\d{1,2}\s+[a-zà-ú]+\s+\d{2,4})/i); if(m2){ const iso=toISODate(m2[1]); if(iso) return iso; }
  return '';
}
...
const BRAND_ALIASES={
  ...
  'saiwa':'Saiwa','san carlo':'San Carlo','ferrero':'Ferrero','motta':'Motta',
  'parmalat':'Parmalat','arborea':'Arborea','de cecco':'De Cecco','kimbo':'Kimbo',
  'pantene':'Pantene','nivea':'Nivea','malizia':'Malizia','vileda':'Vileda'
};
...
function milkAttrs(name=''){
  const s=normKey(name);
  const fat=/\bintero\b/.test(s)?'fat:i':/\b(ps|parzialmente|semi|parz)\b/.test(s)?'fat:ps':/\bscrem\b/.test(s)?'fat:s':'fat:?';
...
function canonicalKey(p){
  ...
  let base=normKey(p.name||'')
    ...
    .replace(/\s+/g,' ').trim();
  const fam=productFamily(p.name||''); if(fam==='fam:pasta') base=base.replace(/\b(\d{3,4})\b/g,' ').trim();
  const milk=/latte\b/.test(base)?('|'+milkAttrs(p.name||'')):'';
  return `${brand}|${base}|${fam}${milk}`;
}
function sanitizeUnits(item){
  const out={...item}; out.brand=canonBrand(out.brand||''); const fam=productFamily(out.name||'');
  if(SUSPECT_UPP.has(Number(out.unitsPerPack||0))||isWeightOrVolumeLabel(out.unitLabel||'')||fam==='fam:pasta'){ out.unitsPerPack=1; out.unitLabel='unità'; }
...
function dedupeAndFix(items=[]){
  ...
  for(const r of items){
    const p=sanitizeUnits(r); const key=canonicalKey(p); const cur=map.get(key);
    if(!cur){ map.set(key,{...p,packs:Math.max(1,Number(p.packs||1)),unitsPerPack:Math.max(1,Number(p.unitsPerPack||1)),unitLabel:p.unitLabel||'unità'}); continue; }
    cur.packs=Math.max(1,Number(cur.packs||1))+Math.max(1,Number(p.packs||1));
    const better=(a,b,aL,bL)=>{ if(aL==='unità'&&bL!=='unità')return[b,bL]; if(bL==='unità'&&aL!=='unità')return[a,aL]; return [Math.max(a,b),aL||bL||'unità']; };
...
async function mapWithLimit(arr, limit, worker){
  const out=new Array(arr.length); let i=0; const running=new Set();
  async function run(k){ running.add(k); try{ out[k]=await worker(arr[k],k);} finally{ running.delete(k); if(i<arr.length) await run(i++);} }
  const n=Math.min(limit,arr.length); for(;i<n;i++) run(i);
  while(running.size) await new Promise(r=>setTimeout(r,10)); return out;
}

/* ===================================================================
   Classificatore OCR + bucket spesa
=================================================================== */
...
/* ===================================================================
   OCR adapters
=================================================================== */
function normalizeItemForPipelinesLight(p){
  return {
...
function normFromOcrHome(j={}){
  const kind=j?.kind||'unknown'; const text=String(j?.text||'');
  const meta={
    store:String(j?.meta?.store||j?.store||''), address:String(j?.meta?.address||j?.address||''),
    place:String(j?.meta?.place||j?.meta?.city||j?.place||j?.city||''), purchaseDate:String(j?.meta?.purchaseDate||j?.purchaseDate||''),
    totalPaid:Number(j?.meta?.totalPaid||j?.totalPaid||0), currency:String(j?.meta?.currency||j?.currency||'EUR')
  };
  const purchases=Array.isArray(j?.purchases)? j.purchases.map(normalizeItemForPipelinesLight):[];
...
function normFromLegacyOcr(j={}){
  ...
  const meta={ store:String(j?.store||''), address:String(j?.address||''), place:String(j?.place||j?.city||''), purchaseDate:String(j?.purchaseDate||''), totalPaid:Number(j?.totalPaid||0), currency:String(j?.currency||'EUR') };
  const src=Array.isArray(j?.purchases)?j.purchases:(Array.isArray(j?.items)?j.items:[]);
  const purchases=src.map(normalizeItemForPipelinesLight);
...
async function fastItemsOnly(file){
  ...
  const r=await fetch('/api/ocrHome?mode=items-only',{method:'POST',body:fd}); const j=await r.json().catch(()=>({}));
  if(r.ok && j?.ok && Array.isArray(j.purchases) && j.purchases.length){ return j.purchases.map(normalizeItemForPipelinesLight); }
  return null;
}
async function fetchOcrUnified(file){
  const quick=await fastItemsOnly(file);
  if(quick && quick.length){ return { kind:'receipt', text:'', meta:{}, purchases: quick.map(normalizeItemForPipelines) }; }
  const fd=new FormData(); fd.append('images',file,file.name||'upload.jpg');
  let r=await fetch('/api/ocrHome',{method:'POST',body:fd}); let j=null; try{ j=await r.json(); }catch{}
  if(r.ok && j && !j.error){ const n=normFromOcrHome(j); if(!(n.kind==='receipt' && n.purchases.length===0)) return n; }
  r=await fetch('/api/ocr',{method:'POST',body:fd}); j=await r.json().catch(()=>({})); return normFromLegacyOcr(j);
}
...
const Home = () => {
  ...
  // TTS
  const [ttsEnabled,setTtsEnabled]=useState(false); const ttsEnabledRef=useRef(false);
  const [voices,setVoices]=useState([]); const [voiceId,setVoiceId]=useState(null);
  const voicesRef=useRef([]); const selectedVoiceRef=useRef(null);
  function loadVoices(){ try{ if(!window?.speechSynthesis) return; const list=window.speechSynthesis.getVoices()||[]; if(!list.length) return;
    ...
    const saved=localStorage.getItem('__tts_voice'); const chosen=ordered.find(v=>v.name===saved)||it[0]||ordered[0]||null;
    setVoiceId(chosen?chosen.name:null); selectedVoiceRef.current=chosen;
  }catch{}}
  useEffect(()=>{ if(!window?.speechSynthesis) return; const on=()=>loadVoices(); window.speechSynthesis.addEventListener('voiceschanged',on); loadVoices(); return()=>window.speechSynthesis.removeEventListener('voiceschanged',on); },[]);
  useEffect(()=>{ try{ if(!voiceId) return; localStorage.setItem('__tts_voice',voiceId); const v=voicesRef.current.find(v=>v.name===voiceId)||null; selectedVoiceRef.current=v; }catch{} },[voiceId]);
  useEffect(()=>{ try{ const saved=localStorage.getItem('__tts_enabled'); const on=saved==='1'; setTtsEnabled(on); ttsEnabledRef.current=on; }catch{} },[]);
  useEffect(()=>{ try{ localStorage.setItem('__tts_enabled', ttsEnabled?'1':'0'); ttsEnabledRef.current=ttsEnabled; }catch{} },[ttsEnabled]);
  function maybeSpeakMessage(msg){
    try{
      if(!(ttsEnabledRef.current||speakModeRef.current)) return;
      const text=String(msg?.text||'').replace(/<[^>]+>/g,'').trim(); if(!text) return;
      const synth=window?.speechSynthesis, Utter=window?.SpeechSynthesisUtterance; if(!synth||typeof Utter!=='function') return;
      const utt=new Utter(text); utt.lang=selectedVoiceRef.current?.lang||'it-IT'; if(selectedVoiceRef.current) utt.voice=selectedVoiceRef.current;
  ...
  function showToast(text,kind='info',ms=2500){
    ...
    setToasts(t=>[...t,{id,text,kind}]); setTimeout(()=>setToasts(t=>t.filter(x=>x.id!==id)),ms);
  ...
  async function runBrainQuery(text, opts={}){
    ...
    if(typeof fn!=='function') return { text:'Modulo brain non disponibile', mono:true };
  ...
  /* =================================================================
     OCR Smart handler
  ================================================================= */
  async function handleSmartOCR(files, { silent=true } = {}){
    const wantSommelier = lastUserIntentRef.current.sommelier || /\b(sommelier|carta (dei )?vini)\b/i.test(queryText);
    try{
      setBusy(true);
      ...
      // c) smista
      const texts=[], receipts=[], labels=[], lists=[];
      ...
      // === Sommelier: carta vini come memoria (no inserimenti) ===
      if(lists.length || (wantSommelier && !labels.length && !receipts.length)){
        const joined=(lists.length?lists.map(x=>x.text||'').join('\n---\n'):texts.join('\n---\n')).trim();
        if(joined){ wineListsRef.current = [...wineListsRef.current, joined].slice(-8); }
        if(!silent){ setChatMsgs(a=>[...a,{role:'assistant',text:'Carta vini acquisita. Ora chiedimi consigli o abbinamenti 🍷'}]); }
        showToast('Carta vini acquisita ✓','success');
        window.dispatchEvent(new CustomEvent('sommelier:memory:update',{detail:{parts:wineListsRef.current.length}}));
        return;
      }

      // === Etichette vino (memoria leggera) ===
      if(labels.length && !receipts.length){
        wineListsRef.current = [...wineListsRef.current, ...labels.map(l=>String(l.text||''))].slice(-12);
        if(!silent){ setChatMsgs(a=>[...a,{role:'assistant',text:'Etichette vino lette. Chiedimi nella modale.'}]); }
        showToast('Etichette vino acquisite ✓','success');
        return;
      }
...
const linkPath  = `${bucket==='cene-aperitivi' ? '/cene' : '/spese-casa'}?rid=${encodeURIComponent(receiptId)}`;
 ...
 await postJSON(endpoint,{
   ...
   receiptTotalAuthoritative: true
          ...
          if(bucket!=='cene-aperitivi' && uid){
            try { await postJSON('/api/stock/apply',{ user_id: userId, items }); }catch(e){ console.warn('Scorte',e); }
          }
      ...
      window.dispatchEvent(new CustomEvent('jarvis:ocr:error',{detail:{message:String(err?.message||err)}}));
  ...
  function openChatWithSystem(initial=[]){
    setChatOpen(true);
    if(initial?.length){
      setChatMsgs(prev=>[...prev, ...initial.map(m=>({role:m.role||'assistant', text:m.text}))]);
    }
  }
  async function askInModal(text){
    ...
    try{
      ...
      const res = await runBrainQuery(q,{ mode:'modal-only', wine_memory: memory || undefined });
      const msg={role:'assistant',text:formatResult(res)};
      setChatMsgs(prev=>[...prev,msg]); maybeSpeakMessage(msg);
    }catch{ const msg={role:'assistant',text:'Errore durante la risposta.'}; setChatMsgs(prev=>[...prev,msg]); maybeSpeakMessage(msg); }
  }

  // Voce → sempre modale
  async function onVoiceText(text){
    const t=String(text||'').trim(); if(!t) return;
    lastUserIntentRef.current={ text:t, sommelier:/\b(sommelier|carta (dei )?vini)\b/i.test(t) };
  ...
  useEffect(()=>{
    ...
    (async ()=>{
      ...
      const imgs=sp.getAll('img'); const q=sp.get('q')||'';
      if(imgs.length){
        const files=[];
        for(let i=0;i<imgs.length;i++){
          try{
            ...
            if(url.startsWith('data:')){
              const [head,b64]=url.split(','); const mime=(head.match(/data:(.*?);base64/i)?.[1])||'image/jpeg';
              const buf=Uint8Array.from(atob(b64),c=>c.charCodeAt(0)); files.push(new File([buf],`deeplink_${i+1}.jpg`,{type:mime}));
            }else{
              const r=await fetch(url); const blob=await r.blob(); files.push(new File([blob],`deeplink_${i+1}.${blob.type.includes('png')?'png':'jpg'}`,{type:blob.type}));
            }
          }catch{}
        }
...
if (!userId) {
  ...
  // UI handlers
  const handleFileChange=(e)=>{ const files=Array.from(e.target.files||[]); if(!files.length||busy) return; (async()=>{ setBusy(true); try{ await handleSmartOCR(files,{silent:true}); } finally { setBusy(false); if(fileInputRef.current) fileInputRef.current.value=''; } })(); };
  const handleSelectOCR=()=>{ if(!busy) fileInputRef.current?.click(); };

  async function submitQuery(textParam){
    const raw=(textParam!=null?String(textParam):queryText).trim(); if(!raw||busy) return;
    if(textParam==null) setQueryText(''); if(!chatOpen) openChatWithSystem();
    await askInModal(raw);
  }
  ...
  function renderInventorySnapshot(payload){
    ...
    const text=`🏠 Scorte (snapshot)\nTotale articoli: ${fmtInt(rows.length)}\n\n${table}`;
    return { text, blocks: barsData.length?[{svg,caption:'Consumo stimato (prime 10 voci)'}]:[] };
  }
  function renderBrainResponse(res){
    if(res && typeof res==='object' && 'text' in res) return {role:'assistant',text:String(res.text??''),mono:!!res.mono};
    const payload=(res&&typeof res==='object' && 'result' in res)?res.result:res;
    const kind=payload?.kind;
    const looksInventory=kind==='inventory.snapshot' || (payload && typeof payload==='object' && Array.isArray(payload.elenco));
    if(looksInventory){ const rendered=renderInventorySnapshot(payload); return {role:'assistant',text:rendered.text,mono:true,blocks:rendered.blocks}; }
    const topList=payload?.top_negozi || payload?.top_stores;
    const looksMonth=(kind==='finances.month_summary' || (payload && typeof payload==='object' && (payload.totale!=null || payload.total!=null) && Array.isArray(topList)));
    if(looksMonth){
      const totRaw=payload.total ?? payload.totale ?? 0; const txs=payload.transactions ?? payload.transazioni ?? 0;
      const top=Array.isArray(topList)?topList:[]; const rows=top.map(r=>({store:r.store||r.nome||r.name||'—', speso:fmtEuro(r.speso??r.amount??0)}));
    ...
    return {role:'assistant',text:formatResult(payload??res),mono:typeof(payload??res)!=='string'};
  }

  return (
    <>
      <Head><title>Home - Jarvis-Assistant</title><meta property="og:title" content="Home - Jarvis-Assistant"/></Head>
      ...
      <main className="home-shell">
        <section className="primary-grid">
          <Link href="/liste-prodotti" className="card-cta card-prodotti animate-card pulse-prodotti sheen"><span className="emoji">🛒</span><span className="title">LISTE PRODOTTI</span><span className="hint">Crea e gestisci le tue liste</span></Link>
          <Link href="/finanze" className="card-cta card-finanze animate-card pulse-finanze sheen" style={{animationDelay:'0.15s'}}><span className="emoji">📊</span><span className="title">FINANZE</span><span className="hint">Entrate, spese e report</span></Link>
        </section>
        ...
        <section className="advanced-box">
          ...
          <div className="advanced-actions">
            ...
            <button className="btn-manuale" onClick={()=>setTtsEnabled(v=>!v)} title="Abilita/Disabilita lettura vocale" aria-pressed={ttsEnabled}>{ttsEnabled?'🔊 Lettura vocale: ON':'🔇 Lettura vocale: OFF'}</button>
            ...
            <VoiceRecorder buttonClass="btn-vocale" idleLabel="🎤 Comando vocale" recordingLabel="⏹ Stop" onText={onVoiceText} disabled={busy}/>
      ...
      {chatOpen && (
        <div style={S.overlay} role="dialog" aria-modal="true" aria-label="Chat dati">
          <div style={S.modal}>
            <div style={S.header}><div style={{fontWeight:800}}>💬 Interroga dati</div><button onClick={()=>setChatOpen(false)} aria-label="Chiudi" style={S.btnGhost}>✖</button></div>
            <div style={S.body}>
              {chatMsgs.length===0 && (<div style={{opacity:.85}}>Inizia chiedendo: “Quanto ho speso questo mese?” • “Che cosa ho a casa?” • “Mi consigli un rosso da questa carta?” (poi premi <b>OCR</b>).</div>)}
              {chatMsgs.map((m,i)=>(
                <div key={i} style={{display:'grid',justifyContent:m.role==='user'?'end':'start'}}><div style={S.bubble}>{m.mono?<pre style={S.pre}>{m.text}</pre>:<span dangerouslySetInnerHTML={{__html:m.text}}/>}</div></div>
              ))}
            </div>
      ...
      <style jsx global>{`
        ...
        .bg-overlay{position:fixed;inset:0;z-index:-1;background:rgba(0,0,0,.35);pointer-events:none}
        ...
        .card-prodotti{background:linear-gradient(145deg, rgba(99,102,241,.85), rgba(236,72,153,.85)); border:1px solid rgba(236,72,153,.35)}
        .card-finanze{background:linear-gradient(145deg, rgba(6,182,212,.85), rgba(59,130,246,.85)); border:1px solid rgba(59,130,246,.35)}
        .animate-card{animation:cardGlow 3.2s ease-in-out infinite}
        @keyframes cardGlow{0%{box-shadow:0 0 15px rgba(99,102,241,.4)}50%{box-shadow:0 0 35px rgba(6,182,212,.85)}100%{box-shadow:0 0 15px rgba(99,102,241,.4)}}
        .advanced-box{width:min(1100px,96vw);margin-top:.5rem;background:rgba(0,0,0,.55);border-radius:16px;padding:1rem}
        ...
        .query-input{width:100%;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.2);border-radius:.55rem;padding:.52rem .7rem;color:#fff;outline:none}
        .query-input::placeholder{color:rgba(255,255,255,.65)}
        .btn-ask{background:linear-gradient(135deg,#6366f1,#06b6d4);border:1px solid rgba(255,255,255,.2);border-radius:.55rem;padding:.45rem .7rem;color:#fff;cursor:pointer}
        ...
        .btn-vocale{background:#6366f1}.btn-ocr{background:#06b6d4}.btn-manuale{background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.2)}
        ...
        .toast.success{border-color:rgba(34,197,94,.5)} .toast.warn{border-color:rgba(234,179,8,.6)} .toast.error{border-color:rgba(239,68,68,.6)}
      `}</style>
    </>
  );
};
