...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!assistantId) {
      return res.status(500).json({ error: 'OPENAI_ASSISTANT_ID non configurato nel deploy' });
    }

    /* 1 - thread + messaggio utente */
    const thread = await openai.beta.threads.create();
    const threadId = thread?.id;
    if (!threadId) throw new Error('Impossibile ottenere threadId');
