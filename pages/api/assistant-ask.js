...
export default async function handler(req, res) {
  ...
  /* Health via GET (non implementato) */
  if (req.method !== 'POST') {
...
/* HEAD aggregator: ledger > categorie; dedup rid o (store+data), prendi MAX */
async function getHeads(uid, start, end) {
  ...
  // fallback categorie (doc_total preferito)
  if (!rows.length) {
...
async function toolSpendSum(uid, { ref='month' } = {}) {
  ...
  const top_stores = [...perStore.entries()].sort((a,b)=>b[1]-a[1]).slice(0,5).map(([store,amount])=>({ store, amount:Number(amount.toFixed(2)) }));
...
async function toolSpendTopProducts(uid, { ref='month', limit=10 } = {}) {
  ...
  const items = [...agg.entries()].sort((a,b)=>b[1]-a[1]).slice(0,limit).map(([name,amount])=>({ name, amount:Number(amount.toFixed(2)) }));
  return { items };
}
...
async function toolStockSnapshot(uid) {
  ...
  if (Array.isArray(scorte)) {
    for (const r of scorte) out.push({ name:r.prodotto||'Articolo', qty:r.quantita_attuale??null, unit:'pz', expiry_date:r.data_scadenza||null });
  }
  const { data: inv } = await sb.from('inventory')
    .select('product_id, qty, unit, expiry_date').eq('user_id', uid);
  if (Array.isArray(inv)) {
    for (const r of inv) out.push({ name:r.product_id||'Item', qty:r.qty??null, unit:r.unit||null, expiry_date:r.expiry_date||null });
  }
  const { data: exp } = await sb.from('expiring_items')
    .select('product_id, qty, unit, expiry_date').eq('user_id', uid);
  if (Array.isArray(exp)) {
    for (const r of exp) out.push({ name:r.product_id||'Item', qty:r.qty??null, unit:r.unit||null, expiry_date:r.expiry_date||null });
  }
  return { items: out.slice(0,50) };
}

/* ─────────────── SCADENZE ENTRO X GIORNI ─────────────── */
...
async function toolShoppingRead(uid) {
  ...
  if (Array.isArray(sh)) for (const r of sh) out.push({
    ...
    qty:Number(r.qty||1), unit:r.unit_label||'unità',
    list_type:r.list_type||'supermercato', category:r.category||'spese-casa'
  });
  ...
  if (Array.isArray(li)) for (const r of li) out.push({
    name:r.product_id || 'Item', qty:Number(r.qty||1), unit:r.unit||'unità',
    store:r.store_name||null, expiry_date:r.expiry_date||null
  });
...
async function toolFinanceBalance(uid, { ref='month' } = {}) {
  ...
  // carryover mese (YYYY-MM dell'end)
  ...
  return {
    start, end, income: inc.total, carryover: carry,
    spend_head: spend.total, balance: Number(balance.toFixed(2))
  };
}
...
async function toolPocketCashSummary(uid, { ref='month' } = {}) {
  ...
  const { data } = await sb.from('pocket_cash')
    .select('delta, amount, direction, moved_date')
    .eq('user_id', uid).gte('moved_date', start).lte('moved_date', end);
  const rows = Array.isArray(data)?data:[];
  let ins=0, outs=0;
  for (const r of rows) {
...
async function toolCellarSummary(uid) {
  const { data } = await sb.from('cellar')
    .select('bottles').eq('user_id', uid);
  const rows = Array.isArray(data)?data:[];
  const bottles = rows.reduce((t,r)=>t + Number(r.bottles||0), 0);
  // esempi rapidi (puoi ampliare)
  return { bottles };
}
...
async function toolCellarWindow(uid, { days_ahead=90 } = {}) {
  ...
  const { data } = await sb.from('cellar')
    .select('wine_id, ready_from, drink_by')
    .eq('user_id', uid);
  const rows = Array.isArray(data)?data:[];
  const ready = [];
  for (const r of rows) {
...
async function toolArtisanSearch(uid, { category=null, text=null, min_aging=null, max_price=null } = {}) {
  let query = sb.from('artisan_products')
    .select('id, name, category, designation, milk_meat_type, aging_days, price_eur, tags, photo_url, notes')
    .eq('user_id', uid);
  if (category) query = query.eq('category', category);
  if (min_aging!=null) query = query.gte('aging_days', Number(min_aging));
  if (max_price!=null) query = query.lte('price_eur', Number(max_price));
  if (text) query = query.or(`name.ilike.%${text}%,designation.ilike.%${text}%`);
  const { data } = await query;
  const items = Array.isArray(data)?data:[];
  return { items };
}

/* ─────────────── PRODUCT PLACES (wine/artisan, origin/purchase) ─────────────── */
async function toolProductPlaces(uid, { item_type=null, term=null, kind=null } = {}) {
  let query = sb.from('product_places')
  ...
  if (term) query = query.ilike('place_name', `%${term}%`);
  const { data } = await query;
  const items = Array.isArray(data)?data:[];
  return { items };
}
...
async function toolOffersSearch(uid, { term=null, days_back=30 } = {}) {
  ...
  const pushNorm = (arr, map) => {
    if (Array.isArray(arr)) for (const r of arr) {
      map.push({
        store_name: r.store_name || r.fonte || null,
        price: Number(r.price || r.prezzo_offerta || 0),
        valid_to: r.valid_to || r.data_scadenza_offerta || null,
        link: r.link_offerta || r.link || null,
        source: r.source || r.fonte || null
      });
    }
  };

  const { data: o1 } = await sb.from('offers')
    .select('store_name, price, valid_to, source, link_offerta, product_id')
    .eq('user_id', uid);
  pushNorm(o1, out);

  const { data: o2 } = await sb.from('offers_weekly_report')
    .select('store_name, price, link_offerta, captured_at').eq('user_id', uid);
  pushNorm(o2, out);

  const { data: o3 } = await sb.from('offerte').select('prodotto, prezzo_offerta, link, fonte, data_scadenza_offerta');
  pushNorm(o3, out);

  // filtro per term (se dato) e data (grossolana)
