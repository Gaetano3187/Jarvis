...
/* ─────────────────────────── UTIL HOISTED ─────────────────────────── */
// disponibili ovunque, anche dentro oaPost/oaGet
function sleep(ms) { return new Promise(r => setTimeout(r, ms)); }
function safeParse(s) { try { return JSON.parse(s || '{}'); } catch { return {}; } }

/* ---------- Pretty-print helper (render testo leggibile dai JSON dei tool) ---------- */
function itDate(iso) {
  if (!iso) return '—';
...
function tryPrettify(text) {
  ...
  let j = null; try { j = JSON.parse(candidate); } catch { return String(text || ''); }
  ...
  // stock_snapshot
  if (j && Array.isArray(j.items) && j.items.length && ('qty' in j.items[0] || 'expiry_date' in j.items[0])) {
  ...
  // price_trend con SVG
  if (j && j.term && j.svg) return `📈 Andamento prezzi “${j.term}”\n\n${j.svg}`;
    ...
    for (let i = 0; i < 48; i++) {
      ...
      if (run.status === 'requires_action') {
        ...
        for (const c of calls) {
          ...
          let out = null;
          try { out = await resolveTool(name, args, userId); }
          catch (e) {
      ...
      if (run.status === 'completed') {
        // prendo più messaggi e cerco il primo con role === 'assistant'
        ...
        console.log('[assistant-ask] chosen message role:', chosen?.role, 'len:', pretty?.length);
...
async function getHeads(uid, start, end) {
  ...
  // 2) HEAD categorie (preferisci doc_total)
  if (!rows.length) {
  ...
  // 4) FALLBACK LEGACY: jarvis_finanze
  if (!rows.length) {
    ...
    if (Array.isArray(jf) && jf.length) {
      const mapped = jf
        ...
        .map(r => ({
          receipt_id: `finz:${r.id}`,
          store: (r.description || 'Finanze').trim(),
...
async function toolStockExpiring(uid, { days_ahead = 7 } = {}) {
  ...
  if (Array.isArray(exp)) for (const r of exp)
    out.push({ name: r.product_id || 'Item', qty: r.qty ?? null, unit: r.unit || null, expiry_date: r.expiry_date });
  ...
  if (Array.isArray(sc)) for (const r of sc)
    out.push({ name: r.prodotto || 'Articolo', qty: r.quantita_attuale ?? null, unit: 'pz', expiry_date: r.data_scadenza });
...
async function toolPocketCashSummary(uid, { ref = 'month' } = {}) {
  ...
  for (const r of rows) {
    const d = (r.delta != null) ? Number(r.delta || 0)
      : (r.amount != null ? ((r.direction === 'in') ? +1 : -1) * Number(r.amount || 0) : 0);
...
async function toolOffersSearch(uid, { term = null, days_back = 30 } = {}) {
  ...
  const out = [];
  const pushNorm = (arr) => {
    if (Array.isArray(arr)) for (const r of arr) out.push({
  ...
  const items = out.filter(x =>
    !term || (x.store_name || '').toLowerCase().includes(norm) || (x.source || '').toLowerCase().includes(norm)
  );
