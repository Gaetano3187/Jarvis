...
function icon(s=''){
  ...
  if(/benzin|carburant|eni|ip|totalerg|agip/.test(v))return '⛽'
  if(/regalo/.test(v))return '🎁'
  if(/elettron|computer|telefon|apple|samsung|mediaworld/.test(v))return '💻'
  if(/sport|gym|piscin|palett/.test(v))return '🏃'
  if(/cinem|teatro|concert|event/.test(v))return '🎬'
  if(/taxi|uber|parcheg|treno|autobus/.test(v))return '🚕'
...
function Varie(){
  ...
  async function onSubmit(e){
    ...
    try{
      ...
      await supabase.from('expenses').insert({user_id:user.id,category:'varie',store:form.store,description:form.description||null,amount:parseFloat(form.amount)||0,purchase_date:form.date||iso(),source:'manual'})
  ...
  async function onAddItem(e){
    ...
    try{
      const{data:{user}}=await supabase.auth.getUser();if(!user)throw new Error()
      await supabase.from('purchase_items').insert({user_id:user.id,category:'varie',name:iForm.name,price:parseFloat(iForm.price)||0,store:iForm.store||null,purchase_date:iForm.date||iso()})
  ...
  async function handleOCR(file){
    ...
    try{
      ...
      if(exp&&data.items?.length){
        ...
        await supabase.from('purchase_items').insert(data.items.map(it=>({user_id:user.id,category:'varie',expense_id:exp.id,name:it.name,price:it.price||0,store:data.store||null,purchase_date:data.purchase_date||iso()})))
      }
      await load(user.id)
    ...
    try{
      ...
      mr.current.onstop=async()=>{
        try{
          ...
          setAiBusy(true)
          const r2=await fetch('/api/assistant-v2',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt:`Registra spesa varia (farmacia, benzina, parrucchiere, regalo): "${j.text}". Azione add_expense category=varie.`,userId,conversationHistory:[]})})
          const d=await r2.json()
          if(d.action?.type==='add_expense'){
            const{data:{user}}=await supabase.auth.getUser()
            await supabase.from('expenses').insert({user_id:user.id,category:'varie',store:d.action.store||'Varie',description:d.action.description||null,amount:Number(d.action.amount||0),purchase_date:d.action.date||iso(),source:'voice'})
            await load(user.id)
  ...
  return(<>
    ...
    <div className="pw"><div className="pi">
      <div className="ph"><div className="pl">JARVIS</div><div className="pp">Spese Varie</div></div>
      <div className="ks">
        <div className="kc"><div className="kl">Totale Varie</div><div className="kv" style={{color:'#94a3b8'}}>{eur(totale)}</div></div>
        <div className="kc"><div className="kl">N° spese</div><div className="kv" style={{color:'#22d3ee'}}>{expenses.length}</div></div>
        <div className="kc"><div className="kl">Acquisti registrati</div><div className="kv" style={{color:'#d4d4d8'}}>{purchases.length}</div></div>
      </div>
      <div className="mc">
        <div className="sh">
          <span className="st">🧰 Spese Varie</span>
          <span className="ss">Farmacia · Benzina · Parrucchiere · Regali</span>
        </div>
        <div className="tr">
          <button className="tb" style={tab==='spese'?{color:'#94a3b8',borderBottom:'2px solid #94a3b8'}:{}} onClick={()=>setTab('spese')}>📋 Spese ({expenses.length})</button>
          <button className="tb" style={tab==='listino'?{color:'#94a3b8',borderBottom:'2px solid #94a3b8'}:{}} onClick={()=>setTab('listino')}>🧾 Acquisti ({purchases.length})</button>
        </div>
        <div className="tl">
          ...
          <button className="bn bn-i" onClick={()=>{setShowItem(v=>!v);setShowForm(false)}}>{showItem?'— Chiudi':'🧾 Aggiungi acquisto'}</button>
        </div>
        {showForm&&<form className="ef" onSubmit={onSubmit}>
          <div className="fr">
            <div className="ff"><label className="fl">Punto vendita</label><input className="fi" value={form.store} onChange={e=>setForm(f=>({...f,store:e.target.value}))} placeholder="Farmacia, ENI, Parrucchiere…" required/></div>
          ...
          <div className="fr">
            <div className="ff"><label className="fl">Descrizione</label><input className="fi" value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))} placeholder="Medicinali, taglio capelli…"/></div>
        ...
        {showItem&&<form className="ef" onSubmit={onAddItem}>
          <div className="fr">
            <div className="ff"><label className="fl">Articolo / Servizio</label><input className="fi" value={iForm.name} onChange={e=>setIForm(f=>({...f,name:e.target.value}))} placeholder="Tachipirina, Taglio capelli…" required/></div>
          ...
          <div className="fr">
            <div className="ff"><label className="fl">Negozio</label><input className="fi" value={iForm.store} onChange={e=>setIForm(f=>({...f,store:e.target.value}))} placeholder="Farmacia Rossi…"/></div>
          ...
          <button type="submit" className="bn bn-s" style={{borderColor:'rgba(148,163,184,.35)',color:'#94a3b8'}}>✓ Aggiungi</button>
        </form>}
        ...
        {tab==='spese'&&<div className="lb">
          {loading?<div className="sk"><span/><span/><span/></div>:expenses.length===0?<div className="le">Nessuna spesa registrata</div>:
          expenses.map(exp=><div key={exp.id} className="eb2">
            <div className="er" onClick={()=>loadDetail(exp.id)}>
              <div className="el">
                <div style={{display:'flex',alignItems:'center',gap:'.3rem'}}>
                  <span style={{fontSize:'.9rem'}}>{icon(exp.store||exp.description||'')}</span>
                  <span className="es" style={{color:'#94a3b8'}}>{exp.store||'—'}</span>
                </div>
            ...
            {expanded===exp.id&&<div className="ed2">
              {recMap[exp.id]?.items?.length>0?(<><div className="dl">🧾 Articoli</div>
                <div className="il">{recMap[exp.id].items.map(it=><div key={it.id} className="ir">
                  ...
                  <span className="ipr" style={{color:'#94a3b8'}}>{eur(it.price)}</span>
                </div>)}</div></>)
        ...
        {tab==='listino'&&<div className="lb">
          {purchases.length===0?<div className="le">Nessun acquisto registrato</div>:
          purchases.map(p=><div key={p.id} className="pr">
            <div className="pico">{icon(p.name||p.store||'')}</div>
            <div className="pn">
            ...
            <div className="pm2">
              <span className="pprice" style={{color:'#94a3b8'}}>{eur(p.price)}</span>
